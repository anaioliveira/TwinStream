...
cd /d D:\Debora\OperationalModels\OPPortugal
...
:: set folder path
set dump_path=\\thredds\thredds\MOHID_Land\Domains\Portugal
...
:: set folder path
set dump_path=D:\Debora\OperationalModels\OPPortugal\Results\Restart
:: remove files from %dump_path%
forfiles -p "%dump_path%" -m *.* -d -%max_days% -c "cmd /c if exist @path del /q @path" 2>nul
:: remove sub directories from %dump_path%
forfiles -p "%dump_path%" -d -%max_days% -c "cmd /c IF @isdir == TRUE if exist @path rd /S /Q @path" 2>nul
...
:: set folder path
set dump_path=D:\Debora\OperationalModels\OPPortugal\Results\Timeseries
:: remove files from %dump_path%
forfiles -p "%dump_path%" -m *.* -d -%max_days% -c "cmd /c if exist @path del /q @path" 2>nul
:: remove sub directories from %dump_path%
forfiles -p "%dump_path%" -d -%max_days% -c "cmd /c IF @isdir == TRUE if exist @path rd /S /Q @path" 2>nul

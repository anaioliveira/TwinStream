...
def manage_nomfich_file(mohid_data_folder, mohid_exe_folder, date1, restart_backup_folder):
    ...
    fin.writelines('BASIN_INI                 : ' + restart_backup_folder + date1 + '\Basin_1' + '.fin\n')
    
    if porous_media == 1:
        fin.writelines('POROUS_INI                : ' + restart_backup_folder + date1 + '\PorousMedia_1' + '.fin\n')
    
    if run_off == 1:
        fin.writelines('RUNOFF_INI                : ' + restart_backup_folder + date1 + '\RunOff_1' + '.fin\n')
    
    if drainage_network == 1:
        fin.writelines('DRAINAGE_NETWORK_INI      : ' + restart_backup_folder + date1 + '\DrainageNetwork_1' + '.fin\n')
    
    if vegetation == 1:
        fin.writelines('VEGETATION_INI            : ' + restart_backup_folder + date1 + '\Vegetation_1' + '.fin\n')
    
    if porous_media_prop == 1:
        fin.writelines('POROUS_PROP_INI           : ' + restart_backup_folder + date1 + '\PorousMediaProperties_1' + '.fin\n')
    
    if run_off_prop == 1:
        fin.writelines('RUNOFF_PROP_INI           : ' + restart_backup_folder + date1 + '\RunOffProperties_1' + '.fin\n')
    
    if reservoirs == 1:
        fin.writelines('RESERVOIRS_INI            : ' + restart_backup_folder + date1 + '\Reservoirs_1' + '.fin\n')
        
    if irrigation == 1:
        fin.writelines('IRRIGATION_INI            : ' + restart_backup_folder + date1 + '\Irrigation_1' + '.fin\n')
...
def init(begin_date, end_date, working_dir):
    ...
    write_log_file(working_dir + '\Logs\mohid_' + date1_ + '.log', stdout)

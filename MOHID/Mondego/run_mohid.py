...
def manage_nomfich_file(continuous, mohid_data_folder, mohid_exe_folder, simulation_number, python_date, time_step, mohid_res_folder, restart_backup_folder):
    ...
    if continuous == 1:
        ...
        if porous_media == 1:
            fin.writelines('POROUS_INI                : ' + mohid_res_folder + 'PorousMedia_' + simulation_number + '.fin\n')
        
        if run_off == 1:
            fin.writelines('RUNOFF_INI                : ' + mohid_res_folder + 'RunOff_' + simulation_number + '.fin\n')
        
        if drainage_network == 1:
            fin.writelines('DRAINAGE_NETWORK_INI      : ' + mohid_res_folder + 'DrainageNetwork_' + simulation_number + '.fin\n')
...
if __name__ == '__main__':
    ...
    # Get the absolute path to the Montargil folder (The "Home" base)
    root_path = os.path.abspath(os.path.dirname(__file__))
    print(f"--- Root Directory set to: {root_path} ---")
    ...
    # --- METEO KEYWORDS (joined with root_path) ---
    glue_meteo_files = read_input_file.glue_meteo_files
    meteo_folder = os.path.join(root_path, read_input_file.meteo_folder)
    boundary_conditions_folder = os.path.join(root_path, read_input_file.boundary_conditions_folder)
    meteo_filename_to_mohid = read_input_file.meteo_filename_to_mohid
    converttohdf_folder = os.path.join(root_path, read_input_file.converttohdf_folder)
    
    # --- MOHID FOLDERS (joined with root_path) ---
    mohid_data_folder = os.path.join(root_path, read_input_file.mohid_data_folder)
    mohid_exe_folder = os.path.join(root_path, read_input_file.mohid_exe_folder)
    mohid_res_folder = os.path.join(root_path, read_input_file.mohid_res_folder)
    
    # --- BACKUP FOLDERS (joined with root_path) ---
    hdf_backup_folder = os.path.join(root_path, read_input_file.hdf_backup_folder)
    timeseries_backup_folder = os.path.join(root_path, read_input_file.timeseries_backup_folder)
    restart_backup_folder = os.path.join(root_path, read_input_file.restart_backup_folder)
    ...
    while actual_python_date < end_python_date:
        # Before doing anything, make sure we are "Standing" in the root path
        os.chdir(root_path)
        ...
        # Run MOHID
        year, month, day = actual_mohid_date.split(' ')[:3]
        meteo_date = year + month + day
        
        print(f"Moving to EXE folder to run MOHID: {mohid_exe_folder}")
        os.chdir(mohid_exe_folder)
        os.system('MOHIDLand.exe > ' + meteo_date + '.log')
        
        # Move back to root before running backups and keyword changes
        os.chdir(root_path)
        ...
        if porous_media == 1 and continuous == 0:
            change_continuous_keyword(mohid_data_folder, 'PorousMedia_' + simulation_number + '.dat')
        ...
        if drainage_network == 1 and continuous == 0:
            change_continuous_keyword(mohid_data_folder, 'DrainageNetwork_' + simulation_number + '.dat')
        ...
        if porous_media_prop == 1 and continuous == 0:
            change_continuous_keyword(mohid_data_folder, 'PorousMedia_Properties_' + simulation_number + '.dat')
            
        if run_off_prop == 1 and continuous == 0:
            change_continuous_keyword(mohid_data_folder, 'RunOff_Properties_' + simulation_number + '.dat')
